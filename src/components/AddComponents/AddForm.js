...
const AddForm = () => {
    ...
    const [checkedStatuses, setCheckedStatuses] = useState(
        new Array(allServices.length).fill(false)
    );
    ...
    const onSubmit = (data) => {
        ...
        Promise.all(temps).then((results) => {
            ...
            // result["services"]=selectCheckBoxData
            const tem = selectCheckBoxData.map(async (val) => {
                console.log(val)
                const response = await CreditCardServices.createModeldata(val, "service")


                return (response.data !== undefined) ? response.data : {}
            })

            Promise.all(tem).then((res) => {
                console.log(res)
    ...
    const handleChecked1 = (e, data, index) => {
        const { checked } = e.target;
    ...
    const totalAmount = selectCheckBoxData.reduce((total, item) => {
        ...
        const amount = parseFloat(item.Price.replace('$', ''));
        return total + amount;
    }, 0);
                                                                ...
                                                                <div className='wizard-card-checkbox-container mb-4'>
                                                                    <div className='wizard-card-checkbox-main'>
                                                                        <div className='wizard-card-checkbox-text-main'>
                                                                            ...
                                                                            <div className='wizard-card-checkbox-action-container'>
                                                                                <Checkbox className='mt-2' checked={checkedStatuses[i]} onChange={(e) => handleChecked1(e, { "ServiceName": val.ServiceName, "Price": val.Price, "Flag": true }, i)}></Checkbox>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
